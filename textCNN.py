...
def transfer(glovedir,word2vec_dir):
    # 训练好的词向量文件
    glove_input_file = datapath(glovedir)
    word2vec_output_file = get_tmpfile(word2vec_dir)  # 创建临时文件
    (count, dimensions) = glove2word2vec(glove_input_file, word2vec_output_file)    #glove转换成word2vec的格式
...
def parse():
    ...
    parser.add_argument('-filter-num', type=int, default=100, help='卷积核的个数')    #为2时，acc=72%
    ...
    # parser.add_argument('-model-dir', type=str, default="./model", help='存储训练模型位置')
...
if __name__ == "__main__":
    ...
    parse=parse()
    params_dir = './model/params_finetune_300.pkl'
    #最好写绝对路径！！！！
    glovedir = '/Users/yanghan/PycharmProjects/torch_learning/text_classify/glove.6B/glove.6B.'+str(parse.embedding_dim)+'d.txt'
    word2vec_dir = '/Users/yanghan/PycharmProjects/torch_learning/text_classify/glove.6B/glove.6B.word2vec.'+str(parse.embedding_dim)+'d.txt'
    if not os.path.exists(word2vec_dir):
        transfer(glovedir, word2vec_dir)
    # 加载词向量模型
    logging.info("加载词向量模型")
    # 使用gensim载入word2vec词向量
    wvmodel = gensim.models.KeyedVectors.load_word2vec_format('./glove.6B/glove.6B.word2vec.'+str(parse.embedding_dim)+'d.txt',
    ...
    train_textcnn_model(net, train_dataloader, parse.epoch, parse.lr)
    t.save(net.state_dict(), params_dir)  # 保存模型参数
    endtrain = time.clock()
    logging.info('train time cost:%f s' % (endtrain - starttrain))
    # 测试
    net = textCNN(parse, vectors=t.FloatTensor(wvmodel.vectors))
    net.load_state_dict(t.load(params_dir))
    textcnn_model_test(net,test_dataloader)
    endtest = time.clock()
    logging.info('test time cost:%f s' % (endtest - endtrain))
    logging.info('overall time cost:%f s' % (endtest - starttime))
